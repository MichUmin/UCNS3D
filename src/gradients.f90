...
SUBROUTINE COMPUTE_GRADIENTS_wall_mean_LSQ_VISCOUS(N,ICONSIDERED,NUMBER_OF_DOG,NUMBER_OF_NEI)!check all
	...
	IF (dimensiona.EQ.3)THEN
		...
		DO VAR2=2,4
			...
			DO TTK=1,NUMBER_OF_DOG
				IF (TTK.NE.K0) THEN
					ATTT=ATTT-ILOCAL_rECON5(ICONSIDERED)%VELOCITYDOF(VAR2-1,TTK)*ILOCAL_RECON3(I)%WALLCOEFF(TTK)
				END IF
			END DO
		...
		DO TTK=1,NUMBER_OF_DOG
			IF (TTK.NE.G0) THEN
				ATTT=ATTT-ILOCAL_rECON5(ICONSIDERED)%GRADIENTSTEMP(TTK)*ILOCAL_RECON3(I)%WALLCOEFG(TTK)
			END IF
		END DO
...
SUBROUTINE COMPUTE_GRADIENTS_wall_turb_LSQ_VISCOUS(N,ICONSIDERED,NUMBER_OF_DOG,NUMBER_OF_NEI) !check_all
	...
	DO VAR2=1,turbulenceequations+passivescalar
		...
		DO TTK=1,NUMBER_OF_DOG
			IF (TTK.NE.K0) THEN
				ATTT=ATTT-ILOCAL_rECON5(ICONSIDERED)%GRADIENTSTURB(1,TTK,VAR2)*ILOCAL_RECON3(I)%WALLCOEFF(TTK)
			END IF
		END DO
		ATTT=ATTT/ILOCAL_RECON3(I)%WALLCOEFF(K0)
